...
export function buildRecommendedParams(
  ...
  const notes: string[] = [
    strategy === "conservative" ? "Conservative source-preserving values were generated." :
    strategy === "more_repair" ? "Stronger values were generated for more audible artifact control." :
    "Slider values were adapted from the analysis."
  ];
...
export function buildRepairOptions(
  ...
  const definitions: Array<{ id: RepairStrategy; label: string; description: string }> = [
    { id: "conservative", label: "Conservative", description: "Source-first. Light cleanup with lower risk of over-repair." },
    { id: "balanced", label: "Balanced", description: "Recommended. Balances artifact reduction and musical weight." },
    { id: "more_repair", label: "More Repair", description: "Stronger. Use when the track still feels sharp or noisy." }
  ];
