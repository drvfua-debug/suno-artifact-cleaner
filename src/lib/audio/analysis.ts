...
export const problemBands = [
  ...
  { key: "presenceHarshness", label: "刺さり", fromHz: 2500, toHz: 5000 },
  { key: "sibilance", label: "歯擦音", fromHz: 5000, toHz: 8500 },
  { key: "metallic", label: "金属感", fromHz: 8500, toHz: 13000 },
  { key: "hiss", label: "高域チリつき", fromHz: 13000, toHz: 18000 },
...
export function analyzeAudio(data: AudioBufferData, chunkSeconds = 10, referenceOverride?: { startSec: number; endSec: number }): AudioAnalysis {
  ...
  if (duration >= 600) warnings.push("10分以上の音源です。ブラウザのメモリ使用量に注意してください。");
  if (data.channels.reduce((sum, ch) => sum + ch.byteLength, 0) > 300 * 1024 * 1024) warnings.push("巨大ファイルです。処理に時間がかかる可能性があります。");
  if (clippingCount(mono) > 0) warnings.push("クリッピング候補が検出されました。");
  return {
    duration,
