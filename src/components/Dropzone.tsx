...
export function Dropzone({ onFile, busy }: Props) {
  ...
  return (
    ...
    >
      ...
      <div>
        <div className="font-semibold">Drop WAV / MP3</div>
        <div className="mt-1 text-sm text-sky-100">After upload, analysis starts automatically and the recommended repair type is selected.</div>
        <div className="mt-1 text-sm text-muted">MP3 is supported for input only; export is WAV. Files are processed in the browser and are not uploaded.</div>
      </div>
      <input
        type="file"
